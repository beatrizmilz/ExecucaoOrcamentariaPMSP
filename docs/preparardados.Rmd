...
```
# Preparando os dados de execução orçamentária da Prefeitura Municipal de São Paulo utilizando R
...
O código a seguir faz o download dos arquivos entre os anos 2003 e 2018, e salva na pasta "data" (essa etapa pode demorar um pouco):
...
- Dados de 2003 à 2017 - download realizado em 26/09/2018;

- Dados de 2008 - download realizado em 28/09/2018;

```{r eval=FALSE, include=TRUE}
orcamento2003 <- readxl::read_xls("data/basedadosexecucao2003.xls")
...
## Padronizar os dados de 2017 e 2018 

Os dados de 2017 e 2018  não estão no mesmo padrão dos anos anteriores. Precisamos padronizar transformando algumas colunas em string/texto.
2017:
```{r eval=FALSE, include=TRUE }
...
exec_orcamentaria_pmsp <-  full_join(orcamento2003,orcamento2004)  
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2005)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2006)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2007)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2008)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2009)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2010)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2011)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2012)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2013)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2014)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2015)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2016)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2017)
exec_orcamentaria_pmsp <-  full_join(exec_orcamentaria_pmsp,orcamento2018)

```

Temos agora um dataframe com os dados de execução orçamentária da Prefeitura Municipal de São Paulo entre os anos 2013 e 2018. Vamos exportar este dataframe para ter um arquivo único:

```{r eval=FALSE, include=TRUE}
write_rds(exec_orcamentaria_pmsp, path="data/exec_orcamentaria_pmsp.rds")
